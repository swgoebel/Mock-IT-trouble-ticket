...
$seperate_words = implode(" ", $keywords);
*/
$representative_id = ($_POST['representative_id']);
	...
	$update_query = "UPDATE Ticket SET status_id = $status_id, representative_id = $representative_id, keywords = '$keywords' WHERE ticket_id = $ticket_id;";
	$date_query = "SELECT date_submitted FROM Ticket WHERE ticket_id='$ticket_id';";
	$keyword_queary = "SELECT keywords FROM Ticket WHERE ticket_id='$ticket_id';";
      $problem_query = "SELECT problem_id FROM Ticket WHERE ticket_id='$ticket_id';";  
		...
		// Connect to MySQL
         ...
         if ( !mysql_select_db( "CSC272_JADE_ARROW", $database ) )
   		...
   		if ( !( $keyword_results = mysql_query( $keyword_queary, $database ) ) )
         die( "Could not keu! <br /></p></div></body></html>" );
		 
		if ( !( $result3 = mysql_query( $update_query, $database ) ) )
         die( "Could not update! <br /></p></div></body></html>" );
	...
	$key_row = mysql_fetch_row( $keyword_results );
...
<div class="content_resize">
 ...
 <table class="table1">
  ...
  <td>
 <table class="table2">
 <td width="78">Incident Number</td>
 <td width="6">:</td>
 <td width="294"><input name="ticket_id" type="text" id="ticket_id"></td>
  </br>
 <tr>
 <td>&nbsp;</td> 
 <td>&nbsp;</td>
...
<form name="form2" method="post" action="reviewticket.php">
 ...
 <td width="6">:</td>
 <td width="294"> <input name="date_submitted" type="text" id="date_submitted" disabled value=<?php 
